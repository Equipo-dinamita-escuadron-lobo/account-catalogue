...
public class TreasuryAccountingService {
    ...
    public AccountingEntry createWriteOff(PayableWriteOffEventDto event){
        ...
        for(var d:event.details()){var payable=resolvePayableAccount(d.payableAccountId(),d.payableAccountCode(),event.enterpriseId());movements.add(AccountingMovement.builder().account(payable.getId()).thirdPartyId(d.supplierId()).debit(d.amount()).credit(BigDecimal.ZERO).description("Baja de obligación "+d.invoiceId()).build());total=total.add(d.amount());}
